    ...
    <div id="box_curso">
		...
		if(mysqli_num_rows($sql_retorno) > 0){	
			...
			if(isset($POST_['alterar'])){
				$id_aluno_post = $_POST['cod_aluno'];
				$data_post = $_POST['dataChamada'];
				$data_post_mod = date('Y-m-d', strtotime($data_post));
				$status_post = $_POST['status'];
				$status_trocado = $status_post === "Presente" ? "0" : "1";
				
				$up_chamada = "UPDATE chamada SET presenca = '$status_trocado' WHERE id_aluno = '$id_aluno_post' AND data_chamada = '$data_post_mod'"; 
			}
					...
					<tr>
						...
						$cor = $status ? 'lightgreen' : 'tomato';?>
							<tr style="background-color: <?php echo $cor;?>">						
								<td> <input value='<?php echo $nome; ?>'>
								<input type='hidden' value='<?php echo $id_aluno; ?>' name='cod_aluno'>
								</td>
								<td> <input value='<?php echo $data; ?>' name='dataChamada'> </td>
								<td> <input value='<?php echo $mascara_status; ?>' name='status' </td>
